...
const flores=[{"name":"margarita", "price":45},{"name":"margarita", "price":45},{"name":"girasol", "price":45}];
 
//ejemplos.forEach((ejemplo)=>console.log(ejemplo.name, ejemplo.price));
let name=[];
let price=[];
flores.forEach((flor)=>name.push(flor.name))
flores.forEach((flor)=>price.push(flor.price))
console.log(name)
console.log(price)
 const newtemplate= `Nombre del producto: ${flores.name} , Precio del producto: ${flores.price}`
 flores.forEach((flor)=>console.log(newtemplate))
